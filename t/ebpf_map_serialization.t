...
subtest '_match_uint_or_uint_array tests' => sub {
    ...
    for my $case (@test_cases) {
        my ($bit_size, $array_size) = ebpf::map::_match_uint_or_uint_array($case->{type});
        is($bit_size, $case->{bitsize}, "Correct bit size for $case->{type}");
        is($array_size, $case->{array_size}, "Correct array size for $case->{type}");
    }
};
...
subtest '_pack_value and _unpack_value tests' => sub {
    my @test_cases = (
        ...
        { type => 'uint16[2]', value => [256, 512], expected => pack('S2', 256, 512) },
        { type => 'uint32', value => 12345, expected => pack('L', 12345) },
        { type => 'uint64', value => 1234567890, expected => pack('Q', 1234567890) },
    );
    ...
    for my $case (@test_cases) {
        ...
        my ($unpacked, $offset) = ebpf::map::_unpack_value($case->{type}, $packed, 0);
        is_deeply($unpacked, $case->{value}, "Correctly unpacked $case->{type}");
    }
};
...
subtest '_serialize and _deserialize tests' => sub {
    ...
    is_deeply($deserialized, $test_data, "Deserialized data matches original");
};
